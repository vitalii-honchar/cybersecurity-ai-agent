...
class TestCurlToolIntegration:
    ...
    async def test_get_with_headers(self):
        ...
        result = await curl_tool(
            '-H "User-Agent: TestAgent/1.0" http://httpbin.org/headers'
        )
    ...
    async def test_post_with_json_data(self):
        ...
        result = await curl_tool(
            f"-X POST -H \"Content-Type: application/json\" -d '{json_data}' http://httpbin.org/post"
        )
    ...
    async def test_authentication(self):
        ...
        result = await curl_tool(
            "-u user:passwd http://httpbin.org/basic-auth/user/passwd"
        )
    ...
    async def test_swagger_documentation_retrieval(self):
        ...
        assert any(
            ...
            for keyword in ["swagger", "openapi", "paths", "definitions"]
        )
    ...
    async def test_common_http_methods(self):
        ...
        post_result = await curl_tool(
            '-X POST -H "Content-Type: application/json" -d \'{"key": "value"}\' http://httpbin.org/post'
        )
