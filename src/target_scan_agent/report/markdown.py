from target_scan_agent.state import TargetScanOutput, Vulnerability, ExposedData, AttackVector
...
def create_markdown_report(scan_results: TargetScanOutput) -> str:
    ...
    if scan_results.risk_assessment:
        ...
        report.append(f"**Overall Risk Level:** {risk_emoji.get(overall_risk, '⚪')} **{overall_risk.upper()}**")
        report.append("")
        report.append(f"**Business Impact:** {scan_results.risk_assessment.business_impact}")
        report.append("")
        report.append(f"**Exploitability:** {scan_results.risk_assessment.exploitability}")
        report.append("")
    ...
    if scan_results.vulnerabilities:
        ...
        for severity in severity_order:
            ...
            if vulns:
                report.append(f"### {severity_emojis[severity]} {severity.upper()} Severity ({len(vulns)})")
                report.append("")
                ...
                for vuln in vulns:
                    ...
                    if vuln.affected_endpoint:
                        report.append(f"**Affected Endpoint:** `{vuln.affected_endpoint}`")
                    report.append("")
    ...
    if scan_results.network_intelligence:
        ...
        if scan_results.network_intelligence.os_fingerprint:
            report.append(f"**Operating System:** {scan_results.network_intelligence.os_fingerprint}")
            report.append("")
        ...
        if scan_results.network_intelligence.open_ports:
            ...
            for port in scan_results.network_intelligence.open_ports:
                ...
                report.append(f"| {port.port} | {port.protocol} | {port.state} | {service} | {version} |")
            report.append("")
    ...
    if scan_results.attack_surface:
        ...
        if scan_results.attack_surface.discovered_endpoints:
            ...
            for endpoint in scan_results.attack_surface.discovered_endpoints:
                ...
                report.append(f"| `{endpoint.url}` | {status} | {content_type} | {size} |")
            report.append("")
        ...
        if scan_results.attack_surface.entry_points:
            ...
            for entry in scan_results.attack_surface.entry_points:
                ...
                report.append(f"#### {risk_emoji.get(entry.risk_level, '⚪')} {entry.name}")
    ...
    if scan_results.exposed_data:
        ...
        for data in scan_results.exposed_data:
            sensitivity_emoji = {"critical": "🔴", "high": "🟠", "medium": "🟡", "low": "🟢", "info": "🔵"}
            report.append(f"### {sensitivity_emoji.get(data.sensitivity, '⚪')} {data.data_type}")
    ...
    if scan_results.remediation_roadmap:
        ...
        # Group by priority
        remediation_by_priority = {"critical": [], "high": [], "medium": [], "low": [], "info": []}
        for item in scan_results.remediation_roadmap:
            remediation_by_priority[item.priority].append(item)
        
        priority_order = ["critical", "high", "medium", "low", "info"]
        priority_emojis = {"critical": "🔴", "high": "🟠", "medium": "🟡", "low": "🟢", "info": "🔵"}
        
        for priority in priority_order:
            items = remediation_by_priority[priority]
            if items:
                report.append(f"### {priority_emojis[priority]} {priority.upper()} Priority")
                report.append("")
                for item in items:
