...
class HttpResult(BaseModel):
    ...
    content_length: Optional[int] = Field(
        default=None, description="Content-Length if available"
    )
    execution_time: float = Field(
        description="Time taken to execute the request in seconds"
    )
    timestamp: datetime = Field(
        default_factory=datetime.now, description="When the request was executed"
    )
    error: Optional[str] = Field(
        default=None, description="Error message if request failed"
    )
    request_headers: Dict[str, str] = Field(
        default_factory=dict, description="Headers sent with the request"
    )
